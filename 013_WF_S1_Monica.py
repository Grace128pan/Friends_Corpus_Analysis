...
# File paths
monica_lines_file_path = r'C:\Users\grace\Desktop\Analysis\Character_Lines\Monica_lines.txt'
...
# Read Monica's lines
with open(monica_lines_file_path, 'r', encoding='utf-8') as file:
    monica_lines = file.read()

# Tokenize the text for common exclude words
tokens_common = clean_and_tokenize(monica_lines, exclude_words_common)
...
plt.title('Top 80 Word Frequency Distribution for Monica (Common Exclude Words)')
plt.xlabel('Words')
...
common_exclude_output_path = os.path.join(analysis_folder_path, 'monica_word_frequency_common_exclude_plot.png')
...
plt.title('Word Cloud for Monica (Common Exclude Words)')
plt.axis('off')
plt.tight_layout()

# Save the word cloud for common exclude words
common_exclude_wordcloud_output_path = os.path.join(analysis_folder_path, 'monica_wordcloud_common_exclude.png')
