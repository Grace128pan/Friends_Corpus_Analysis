...
# File paths
input_file_path = "C:/Users/grace/Desktop/Analysis/S10_rachel/rachel_lines_season10.txt"
...
filtered_bigram_frequencies = {tuple(bigram): freq for bigram, freq in bigram_frequencies.items() if freq > 3}
...
plt.title('Bigram Frequencies (Over 3)')
plt.xlabel('Bigrams')
plt.ylabel('Frequency')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
...
# Plotting trigram frequencies (20 most common)
plt.figure(figsize=(15, 5))
filtered_trigram_frequencies = {tuple(trigram): freq for trigram, freq in trigram_frequencies.items() if freq > 3}
...
plt.title('Trigram Frequencies (Over 3)')
plt.xlabel('Trigrams')
plt.ylabel('Frequency')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
