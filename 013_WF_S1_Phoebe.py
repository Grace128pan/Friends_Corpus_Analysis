...
# File paths
phoebe_lines_file_path = r'C:\Users\grace\Desktop\Analysis\Character_Lines\Phoebe_lines.txt'
...
# Read Phoebe's lines
with open(phoebe_lines_file_path, 'r', encoding='utf-8') as file:
    phoebe_lines = file.read()

# Tokenize the text for common exclude words
tokens_common = clean_and_tokenize(phoebe_lines, exclude_words_common)
...
plt.title('Top 80 Word Frequency Distribution for Phoebe (Common Exclude Words)')
plt.xlabel('Words')
...
common_exclude_output_path = os.path.join(analysis_folder_path, 'phoebe_word_frequency_common_exclude_plot.png')
...
plt.title('Word Cloud for Phoebe (Common Exclude Words)')
plt.axis('off')
plt.tight_layout()

# Save the word cloud for common exclude words
common_exclude_wordcloud_output_path = os.path.join(analysis_folder_path, 'phoebe_wordcloud_common_exclude.png')
